...
fname       <- "P:/3015039.05/data/all_sub/eyes_virt_bin_visual_stimlock_5bin.csv"
...
emmeans(model_acc, pairwise ~ eye)
...
p1 <- ggplot(tgc, aes(x=bin, y=perc_corr)) + 
  ...
  ylim(0.7,0.8)+theme_clean()+ggtitle(paste0("vis alpha and accuracy p = ",round(model_acc_anova$`Pr(>F)`[2],2)))
...
p2 <- ggplot(tgc, aes(x=bin, y=med_rt)) + 
  ...
  ylim(0.6,0.7)+theme_clean()+ggtitle(paste0("vis alpha and RT p = ",round(model_rt_anova$`Pr(>F)`[2],2)))
...
p3 <- ggplot(tgc, aes(x=bin, y=perc_corr,group=eye,color=eye)) + 
  ...
  ylim(0.7,0.8)+theme_clean()+ggtitle("vis alpha and accuracy")
...
p4 <- ggplot(tgc, aes(x=bin, y=med_rt,group=eye,color=eye)) + 
  ...
  geom_errorbar(aes(ymin=med_rt-se, ymax=med_rt+se), width=error_size, size=line_size,position=pd) +
  scale_colour_manual(values = cbPalette_eyes) +scale_fill_manual(values = cbPalette_eyes)+
  ylim(0.6,0.7)+theme_clean()+ggtitle("vis alpha and RT")
