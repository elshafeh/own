...
ext2           <- "ageing_contrast_broadmann_alpha_cue_effect_minusevoked_8and12_sep_time"
pat            <-  read.table(paste0(ext1,ext2,".txt"),header=T)

pat            <- pat[pat$MOD == "aud",];pat$CHAN       <- factor(pat$CHAN);pat$MOD        <- factor(pat$MOD)

model.pat_1    <- lme4::lmer(POW ~ (GROUP+HEMI+CUE+FREQ)^4 + (1|SUB), data =pat)
model_anova_1  <- Anova(model.pat_1,type=2,test.statistic=c("F"))

model.pat_2    <- lme4::lmer(POW ~ (GROUP+HEMI+CUE_POSITION+FREQ)^4 + (1|SUB), data =pat)
...
lsmeans::cld(lsmeans::lsmeans(model.pat_3,  pairwise~CUE_POSITION|GROUP),details= TRUE)
lsmeans::cld(lsmeans::lsmeans(model.pat_4,  pairwise~CUE_POSITION|GROUP),details= TRUE)


sub_pat        <- pat[ pat$HEMI == "L_Hemi" & pat$FREQ == "12Hz",]
...
sub_pat        <- pat[ pat$HEMI == "R_Hemi" & pat$FREQ == "12Hz",]
tgc <- summarySE(sub_pat, measurevar="POW", groupvars=c("CUE_POSITION","GROUP"))
