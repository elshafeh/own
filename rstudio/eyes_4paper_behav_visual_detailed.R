...
alldata           <- alldata[alldata$band == "alpha" &  alldata$measure == "visual power",]
...
for (ncom in 1:length(list_compare)){
  ...
  pplot <- ggplot(rep_data, aes(x = eye, y = var, fill = behavior)) +
    ...
    scale_y_continuous(name = 'visual alpha power',limits = c(-2,2),
...
ggsave(filename="/Users/heshamelshafei/Dropbox/project_me/presentations/eyes2021/_svg/eyes_visual_alpha_eqlimits.svg",
