...
fname               <- paste0(dir_file,"eyes_virt_stimlock_behav2fft_balance_withbroadman.csv")
...
list_window         <- unique(levels(sub_table$window)) # 
list_roi            <- unique(levels(sub_table$roi))

# plot
list_maps           <- c("Dark2","Dark2","Dark2","Dark2","Dark2","Dark2","Dark2","Dark2")
...
list_compare        <- unique(sub_table$compare)
...
for (ncom in 1:length(list_compare)){
  ...
  pval2 <- round(model_beh_anova$`Pr(>F)`[2],3)
  pval3 <- round(model_beh_anova$`Pr(>F)`[3],3)
  ...
  p1                  <- ggplot(rep_data, aes(x = eye, y = var, fill = cond))+
    ...
    scale_y_continuous(name = "(Contra-Vis) / (Contra+Vis)",limits=c(-1.1,1.1))+
...
grid.arrange(grobs = myplots, ncol = 3,nrow,1)