...
fname                 <- "P:/3015039.05/data/all_sub/eyes_virt_bin_lat_index_withcue.csv"
sub_table             <- read.table(fname,sep = ',',header=T)

model_acc             <- lme4::lmer(perc_corr ~ (eye+bin+roi)^2 + (1|sub), data =sub_table)
model_acc_anova       <- Anova(model_acc,type=2,test.statistic=c("F"))
print(model_acc_anova)

emmeans(model_acc, pairwise ~ eye)
emmeans(model_acc, pairwise ~ nbin)

model_rt              <- lme4::lmer(med_rt ~ (eye+bin+roi)^2 + (1|sub), data =sub_table)
...
p1 <- ggplot(tgc, aes(x=bin, y=perc_corr)) + 
  ...
  ylim(0.7,0.8)+theme_clean()+ggtitle("lat index and accuracy")
...
p2 <- ggplot(tgc, aes(x=bin, y=med_rt)) + 
  ...
  ylim(0.6,0.7)+theme_clean()+ggtitle("lat index and RT")
