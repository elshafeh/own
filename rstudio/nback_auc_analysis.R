...
data_table          <- read.table("/Users/heshamelshafei/Dropbox/project_me/doc/nback/nback.auc.4anova.txt",
                          sep = ',',header=T)

model_lm            <- lme4::lmer(auc ~ (cond+band+stim)^3 + (1|sub), data =data_table,REML = TRUE,
                                  control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
model_anova         <- Anova(model_lm,type=2,test.statistic=c("F"))
print(model_anova)

model_lm            <- lme4::lmer(auc ~ (cond+band+stim)^3 + (cond|sub), data =data_table)
model_anova         <- Anova(model_lm,type=2,test.statistic=c("F"))
print(model_anova)

model_lm            <- lme4::lmer(auc ~ (cond+band+stim)^3 + (band|sub), data =data_table)
model_anova         <- Anova(model_lm,type=2,test.statistic=c("F"))
print(model_anova)
