...
ext_peak          <- "sensor"
fname             <- paste0(dir_file,"eyes_virt_stimlock_behav2fft.concat.",ext_peak,".peaks.500pre.csv")

alldata           <- read.table(fname,sep = ',',header=T)
alldata           <- alldata[alldata$band == "alpha" &  alldata$measure == "somatovisual balance",]
...
for (ncom in 1:length(list_compare)){
  ...
  # model_beh         <- lme4::lmer(var ~ (eye+behavior+roi)^2 + (1|sub), data =rep_data)
  ...
  pplot <- ggplot(rep_data, aes(x = eye, y = var, fill = behavior)) +
    ...
    scale_y_continuous(name = 'Balance',limits = c(-2,2),
...
ggsave(filename="/Users/heshamelshafei/Dropbox/project_me/presentations/eyes2021/svg/eyes_balance_alpha.svg",
