...
ext2           <- "PrepAtt2_Inf_minus_index_age_contrast_separateROIs_two_Freq_Sep_Time.txt"
pat            <-  read.table(paste0(ext1,ext2),header=T)
pat            <- pat[pat$TIME != "1000ms",] ; pat$TIME       <- factor(pat$TIME)

model.pat      <- lme4::lmer(POW ~ (GROUP+CUE+HEMI+FREQ_CAT)^3 + (1|SUB), data =pat)
...
lsmeans::cld(lsmeans::lsmeans(sub_model.pat,  pairwise~CUE|HEMI),details= TRUE)
