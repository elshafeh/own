...
#fname = "/Users/heshamelshafei/Dropbox/Fieldtripping/R/txt/PrepAtt22_Allparticipants_behav_table4R_withTukey.csv"
fname = "/Users/heshamelshafei/Google Drive/PhD/Fieldtripping/R/txt/PrepAtt22_behav_table4R_withTukey.csv"
...
group_anova <- ezANOVA(pat_age,dv=.(MedianRTukey),wid = .(SUB),within = .(CUE,DIS),between = (GROUP),detailed=T)
print(group_anova)
...
interaction.ABC.plot(MedianRTukey, x.factor=DIS,
                     ...
                     data=pat_age, c,ggplotFunc=list(ylim(400,700),geom_errorbar(data=tgc,
                                                                                 aes(ymax=MedianRTukey+se, ymin=MedianRTukey-se),width=0.2)))
...
group_model.pat <- lme4::lmer(MedianRTukey ~ (GROUP+CUE+DIS)^3 + (1 | GROUP/SUB), data =pat_age)
group_model_anova         <-Anova(group_model.pat,type=2,test.statistic=c("F"))
print(group_model_anova)
lsmeans::cld(lsmeans::lsmeans(group_model.pat,  pairwise~DIS|GROUP))
