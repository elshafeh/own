...
fname                 <- paste0(dir_file,"eyes.behav.fft.clean.lat.1s.csv")
...
for (ncom in 1:length(list_compare)){
  ...
  rep_data$sub        <- factor(rep_data$sub)
  ...
  for (neyes in 1:length(list_eyes)){
    ...
    limit_open <- c(-1,1) # c(-0.8,2)#
    limit_close <- limit_open #limit_open#
...
ggsave(filename="/Users/heshamelshafei/Dropbox/project_me/figures/eyes/eyes_final_lat.svg",
       plot=fullfig,width=10,height=3)

# outlier_value = 0.49
# find_outlier = alldata[alldata$value > outlier_value,]
# bad_suj <- unique(find_outlier$sub)
# 
# 
# # # cap outliers
# # x                   <- rep_data$var
