...
fname       <- "P:/3015039.05/data/all_sub/eyes_virt_bin_visual_stimlock_withemi.csv"
sub_table   <- read.table(fname,sep = ',',header=T)

model_acc       <- lme4::lmer(perc_corr ~ (eye+bin+hemi)^2 + (1|sub), data =sub_table)
...
model_rt        <- lme4::lmer(med_rt ~ (eye+bin+hemi)^3 + (1|sub), data =sub_table)
model_rt_anova  <- Anova(model_rt,type=2,test.statistic=c("F"))
print(model_rt_anova)

emmeans(model_rt, pairwise ~ bin)
