...
for nsuj = 1:length(suj_list)
    ...
    list_cond                           = {'correct','button','match'};
    
    for n_con = 1:length(list_cond)
        
        fname                           = [dir_data subjectName '.decodeLockRep.' list_cond{n_con} '.dwn100.all.auc.mat'];
    ...
    alldata{nsuj,flg}.avg                 = vct; clear vct;
    
end

keep alldata list_*

list_color                              = repmat('b',length(list_cond),1);
...
for nt = 1:length(stat)
    ...
    for nchan = 1:length(stat{nt}.label)
        ...
        if ~isempty(ix)
            ...
            cfg.z_limit                 = [z_limit_low z_limit_high(nt)];
