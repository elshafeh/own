...
for ngrp = 1:length(suj_group)
    ...
    lst_time    = {'p600p1000'};
    lst_bsl     = 'm600m200';
    ext_comp    = 'dpssFixedCommonDicSource80Slct0.5cm.mat';
        ...
        for cnd_freq = 1:length(lst_freq)
            for cnd_time = 1:length(lst_time)
                for ncue = 1:length(lst_sub_cond)
                    ...
                    fname = ['../data/' suj '/field/' suj '.' lst_sub_cond{ncue} cond_main '.' lst_freq{cnd_freq} '.' lst_bsl '.' ext_comp];
                    fprintf('Loading %50s\n',fname);
                    load(fname);
                    
                    bsl_source                                            = source; clear source
                    
                    fname = ['../data/' suj '/field/' suj '.' lst_sub_cond{ncue} cond_main '.' lst_freq{cnd_freq} '.' lst_time{cnd_time}   '.' ext_comp];
                    fprintf('Loading %50s\n',fname);
                    load(fname);
