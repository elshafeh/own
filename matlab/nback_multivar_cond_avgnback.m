    ...
    for nband = 1:length(list_band)
        for nlock = 1:length(list_lock)
            ...
            for nfreq = 1:length(list_freq)
                ...
                dir_data            = 'J:/nback/sens_level_auc/cond/';
                suj_name            = ['sub' num2str(suj_list(nsuj))];
                fin_ext             = '.dwn70.bsl.excl.auc.mat';
                
                file_list         	= dir([dir_data suj_name '.decoding.1back.agaisnt.all.' num2str(list_freq(nfreq)) 'Hz.lockedon.' list_lock{nlock} fin_ext]);
                file_list         	= [file_list; dir([dir_data suj_name '.decoding.2back.agaisnt.all.' num2str(list_freq(nfreq)) 'Hz.lockedon.' list_lock{nlock} fin_ext])];
            ...
            list_cond{i}        	= [list_lock{nlock} ' ' list_band{nband}(1:2)];
            fprintf('\n');
