...
for nsuj = [1:33 35:36 38:44 46:51]
    ...
    for nstim = [1 2]
        ...
        for ntime = [1]
            ...
            if nstim < 3
                cfg.trials	= find(trialinfo(:,2) == nstim & rem(trialinfo(:,4),2) ~= 0); % target or first + correct
            else
                cfg.trials 	= find(trialinfo(:,2) > -1 & rem(trialinfo(:,4),2) ~= 0); % all + correct
            end
            ...
            fname_out     	= [dir_data 'sub' num2str(nsuj) '.allback.' ext_behav '.' list_stim{nstim} '.' list_time_name{ntime}  '.fft.mat'];
        ...
        fname_out           = [dir_data 'sub' num2str(nsuj) '.allback.' ext_behav '.' list_stim{nstim} '.mtm.mat'];
