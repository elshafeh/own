...
for nsuj = 1:length(suj_list)
    ...
    for nsession = 1:2
        ...
        apeak                               = round(allpeaks(nsuj,2));
        com_filter                          = nbk_common_filter(data,leadfield,vol,[-1 2],apeak,2); clc;
        ...
        for nc = 1:length(list_name)
            ...
            if ~isempty(cfg.trials)
                ...
                list_time                   = [-1 0; 0.2 1.2; 0.15 1.15];
                ...
                for ntime = 1:size(list_time,1)
                    ...
                    [source,ext_name]       = nbk_dics_separate(sub_data,leadfield,vol,com_filter,[t1 t2],apeak,2);
                    
                    fname_out               = ['J:/temp/nback/data/source/beta/' subjectname '.session' num2str(nsession) '.' list_name{nc} '.' ext_name  '.dics.mat'];
                    fprintf('\nsaving %s\n',fname_out);
