...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [MI_matrix_raw,MI_matrix_surr,MI_raw_surr,MI_raw_surr_std] = separate_calc_MI_optimised(original_virtsens_pha,original_virtsens_amp,list_toi, ...
    phase,amp,diag,surrogates,list_approach,step_phase,step_amp,chan_index,trial_index,remove_evoked,varargin)
    ...
    for han_chan = 1:n_chan_out
        for han_cond = 1:n_cond_out
            for han_meth = 1:length(n_meth_out)
                ...
                MI_raw_surr_std{han_chan,han_cond,han_toi,han_meth}     = zeros(amp_length,phase_length);
            ...
            for han_meth = 1:n_meth_out
                ...
                for han_phase_i = 1:iphase
                    for han_ampl_i = 1:iampl
                        ...
                        lmt1                                = find(round(virtsens_pha{han_phase_i}.time{1},3) == round(list_toi(1,han_toi),3));
                        lmt2                                = find(round(virtsens_pha{han_phase_i}.time{1},3) == round(list_toi(2,han_toi),3));
                        
                        trial_in                            =  trial_index{han_cond};
                        
                        if strcmp(remove_evoked,'yes')
                            evoked_pha                          = cat(3,virtsens_pha{han_phase_i}.trial{trial_in});
                            evoked_pha                          = mean(squeeze(evoked_pha(chan_index(han_chan),lmt1:lmt2,:)),2)';
                            
                            evoked_amp                          = cat(3,virtsens_amp{han_ampl_i}.trial{trial_in});
                            evoked_amp                          = mean(squeeze(evoked_amp(chan_index(han_chan),lmt1:lmt2,:)),2)';
                        end
                        ...
                        for trial_num = 1:length(trial_in)
                            ...
                            trial_raw_pha = virtsens_pha{han_phase_i}.trial{1, trial_in(trial_num)}(chan_index(han_chan),lmt1:lmt2);
                            trial_raw_amp = virtsens_amp{han_ampl_i}.trial{1, trial_in(trial_num)}(chan_index(han_chan),lmt1:lmt2);

                            if strcmp(remove_evoked,'yes')
                                virtsens_pha{han_phase_i}.trial{1, trial_in(trial_num)}(chan_index(han_chan),lmt1:lmt2) = trial_raw_pha - evoked_pha;
                                virtsens_amp{han_ampl_i}.trial{1, trial_in(trial_num)}(chan_index(han_chan),lmt1:lmt2) = trial_raw_amp - evoked_amp;
                            end
                        ...
                        if strcmp(surrogates, 'yes')
                            ...
                            MI_raw_surr_std{han_chan,han_cond,han_toi,han_meth}(row1,row2)  = std(MI_surr);
