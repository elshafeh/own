...
for sub = 1:21
    ...
    for ncue = 1:length(list_cue)
        ...
        for chan = 1:length(original_virtsens_pha.label)
            ...
            cfg                         = [];
            cfg.channel                 = chan;
            cfg.trials                  = trial_array{ncue}; %h_chooseTrial(original_virtsens_pha,list_ix_cue{ncue},list_ix_dis{ncue},list_ix_tar{ncue});
            ...
            chan_slct_virtsens_pha      = h_removeEvoked(chan_slct_virtsens_pha);
            chan_slct_virtsens_amp      = h_removeEvoked(chan_slct_virtsens_amp);
            ...
            for nme = 1:length(list_method)
                for ntime = 1:size(list_time,2)
                    ...
                    fname_ext1  = ['../data/' suj '/field/' suj '.' list_cue{ncue} '.' ext_essai '100SlctMinEvoked'];
