function r_PrepAtt22_averageERP(ext)
...
for ev = 1:length(event_names) %bp, cue, dis, fdis, target
    ...
    mkdir(['../data/avg_mig/erp/' data '/' event_names{ev}]);
    
    for cat = 1:length(event_list)
        
        if exist(['../data/' list_ctl{1} '/erp/' data '/' event_names{ev} '/' list_ctl{1} '.pat22.' event_list{cat} ext '.p']) == 2;
            
            cd(['../data/avg_mig/erp/' data '/' event_names{ev}])
...
list_mig    = {'mg1' 'mg2' 'mg3' 'mg4' 'mg5' 'mg6' 'mg7' 'mg8' 'mg9' ...
    'mg10' 'mg11' 'mg12' 'mg13' 'mg14' 'mg15' 'mg16' 'mg17' 'mg18' 'mg19'};
list_ctl    = {'uc6' 'uc10' 'yc17' 'yc19' 'uc8' 'uc7' 'uc9' 'yc12' ...
    'yc6' 'uc1' 'uc4' 'yc5' 'yc20' 'yc21' 'yc16' 'yc18' 'yc4'};
load ../documents/event.mat; %structure 'event'
event_names = fieldnames(event);
%ext         = '.lb';
%ext         = '.lb';
