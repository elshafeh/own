...
for nsuj = 1:length(suj_list)
    ...
    for nfreq = 1:length(frequency_list)
        ...
        %
        %             orig_time_axis          = load([dir_data 'sub001.1stcue.lock.theta.minus1f.centered.decodingresp.correct.auc.mat'],'time_axis');
        ...
        t2                      = find(round(avg.time,2) ==round(6.5,2));
...
for nchan = 1:length(stat.label)
    ...
    zlimit              	= [0.47 0.6;0.47 0.6;0.47 0.6];
    ...
    if strcmp(lock_focus,'response')
        vline([-4 -2.5 -1 0],'--k');
        xticks([-4 -2.5 -1 0]);
        xticklabels({'1st Gab' '2nd Cue' '2nd Gab' 'Response Onset' '0.5'});
    else
    ...
    zlimit                          = [0.47 0.55;0.47 0.55;0.47 0.55];
