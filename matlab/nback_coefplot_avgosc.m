...
for nsuj  = 1:length(list_suj)
    ...
    list_cond                                   = {'*.peak.centered.1back.isfirst','*.peak.centered.2back.isfirst', ...
        '*.peak.centered.1back.istarget','*.peak.centered.2back.istarget'};
    
    for ncond = 1:length(list_cond)
        
        sub_carr                              	= [];
        flist                                   = dir(['J:/nback/source/coef/sub' num2str(list_suj(nsuj)) '.' list_cond{ncond} '.bsl.exl.coef.combinedlead.lcmv.mat']);
...
load ../data/stat/nback.multivar.avgnback.mat
...
for ncluster = 1:length(cluster_list)
    for ntest = 1:size(list_test,1)
        ...
        for nview = 1:length(list_view)
            ...
            fout = ['D:/Dropbox/project_me/figures_me/nback/post-kia/source/multivar/osc_avg/test' num2str(ntest) '.' list_view_name{nview} '.png'];
            saveas(gcf,fout);
