...
for nfreq = 1:length(flist)
    for ntime = 1:length(tlist)
        for ncue = 1:length(list_ix_cue_side)
            ...
            cfg.method                  = 'pcc';
            cfg.grid                    = leadfield;cfg.grid.filter = com_filter;
            cfg.headmodel               = vol;
            cfg.keeptrials              = 'yes';
            cfg.pcc.lambda              = '5%';
            ...
            for nmeth = 1:length(list_method)
                ...
                for nroi = 1:length(list_H)
                    ...
                    fname   = ['../data/paper_data/' suj '.pt' num2str(prt) '.' list_ix_cue_side{ncue} cond_main '.' ext_time '.' ext_freq '.' new_name_extra '.' list_H{nroi} '.' cfg.method 'Conn.' ext_index '.mat'];
