...
for nsuj = [1:33 35:36 38:44 46:51]
    ...
    data                                    = ft_appenddata([],sess_carr{2}); clear sess_carr;
    ...
    for nstim = [1 2]
        for nback = [1 2]
            ...
            if nstim == 1
                % target
                cfg.trials           	= find(trialinfo(:,1) == nback + 4 & trialinfo(:,2) == 2);
            elseif nstim == 2
                % allstim
                cfg.trials           	= find(trialinfo(:,1) == nback + 4);
            end
            ...
            fname_out                   = [fname_out '.itc.withevoked.equal.2session.mat'];
            fprintf('Saving %s\n',fname_out);
