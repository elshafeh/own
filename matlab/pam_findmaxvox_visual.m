...
for nsuj = 2:21
    ...
    list_name                 	= 'CnD';
    list_time               	= {'m200m0ms' 'm0p200ms'};
    
    dir_in                      = '~/Dropbox/project_me/data/pam/source/';
    fname_in                    = [dir_in subjectName '.' list_name '.' list_time{1} '.lcmvsource.mat'];
    fprintf('loading %s\n',fname_in);
    load(fname_in);
    bsl                         = source;
    
    fname_in                    = [dir_in subjectName '.' list_name '.' list_time{2} '.lcmvsource.mat'];
    fprintf('loading %s\n',fname_in);
    load(fname_in);
    act                         = source;
    ...
    save(['~/Dropbox/project_me/data/pam/vox/' subjectName '.maxvox.visual.mat'],'vox_list_l','vox_list_r');
