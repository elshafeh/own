function data = bil_changelock_2ndgab(subjectName,new_time_window,data_in)
...
cfg.begsample                           = offset.probe; % target is 1st and probe is second :/
data                                    = h_redefinetrial(cfg,data_in);

data.trialinfo                          = [data.trialinfo repmat(1,length(data.trialinfo),1) code.probe];