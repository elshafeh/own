...
cmp_array           = {[-0.1 -0.07],[-0.1 -0.07]};
twin_array          = {[0.1 0.07],[0.1 0.07]};

cnd_array(1).cue    = {0:2,0:2,[1 2],0};
cnd_array(1).tar    = {1:4,1:4,1:4,1:4};
cnd_array(1).dis    = {1,3,1:3,1:3};
cnd_array(1).ext    = {'DIS1','DIS3','VDIS','NDIS'};

cnd_array(2).cue    = {0:2,0:2,[1 2],0};
cnd_array(2).tar    = {1:4,1:4,1:4,1:4};
cnd_array(2).dis    = {1,3,1:3,1:3};
cnd_array(2).ext    = {'fDIS1','fDIS3','fVDIS','fNDIS'};
        ...
        for cnd_big = 1:length(con_array)
            ...
            for cnd_small = 1:length(cnd_array(cnd_big).ext)
                ...
                for ncomp = 1:size(cmp_array{cnd_big},2)
                    ...
                    fname_out               = [suj '.pt' num2str(prt) '.' cnd_array(cnd_big).ext{cnd_small} '.'  ext_time 'ms.lcmvSource'];
                    
                    fprintf('\n\nSaving %50s \n\n',fname_out); save(['../data/source/' fname_out '.mat'],'source','-v7.3');
