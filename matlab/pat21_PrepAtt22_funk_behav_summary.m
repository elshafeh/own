function [trl_tot,behav_summary]  = PrepAtt22_funk_behav_summary(behav_in_recoded)
...
h = waitbar(0,'Analysing performances ...');
...
for n = 1:length(behav_in_recoded)
    ...
    if  floor(behav_in_recoded(n,3)/1000)==1
        ...
        [sub_idx,nbloc,ntrl_blc,code,CUE,DIS,TAR,XP,REP,CORR,RT,ERROR,cue_idx,CT,DT,cueON,disON,tarON,CLASS,idx_suj] = deal(0);
        ...
        if size(trl,1) > 1
            ...
            behav_summary = [behav_summary ; sub_idx nbloc ntrl_blc code CUE DIS TAR XP REP CORR RT ERROR cue_idx CT DT cueON disON tarON CLASS idx_suj ntrl];
