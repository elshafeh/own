...
for nsuj = 1:length(suj_list)
    ...
    cue_list                = {'pre.freq' 'pre.ori' 'retro.*'}; 
        ...
        for ncue = 1:length(cue_list)
            ...
            for nlock = 1:length(lock_list)
                for ndeco = 1:length(decoding_list)
                    ...
                    flist       = dir(['F:/bil/decode/' subjectName '.1stgab.lock.' frequency_list{nfreq} ...
...
for nfreq = 1:size(alldata,2)
    ...
    stat{nfreq}                         = ft_timelockstatistics(cfg, alldata{:,nfreq,1}, alldata{:,nfreq,2}, alldata{:,nfreq,3});
