...
for nsuj = 1:length(suj_list)
    ...
    for nevo = {'withEvoked' 'minusEvoked'}
        
        fname                       = [project_dir 'data/' sujName '/tf/' sujName '.cuelock.itc.5binned.' nevo{:} '.withIncorrect.mat'];
        fprintf('loading %s\n',fname);
        load(fname);
        
        list_bin                	= {'bin1w' 'bin5w' 'bin1e' 'bin5e'};
...
%%
close all;
list_title                      = {'with evoked' 'minus evoked'};
...
for ntest = 1:2
    ...
    iplot                       = iplot+1;
    subplot(nrow,ncol,list_plot(iplot));
    ft_topoplotTFR(cfg,statplot);
    title(list_title{ntest});
    ...
    list_multip                 = [0.1 0.04];
    list_y                  	= [0.02 0.1; 0.018 0.04];

    fnd_time(fnd_time ~= 0)   	= list_multip(ntest);
