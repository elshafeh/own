    ...
    for ncond = 1:length(list_cond)
        for ntime = 1:length(list_window)
            ...
            flist                            = dir(['I:\bil\source\' subjectName '.*Hz.m1000m0'  ...
                '.' list_cond{ncond} '.AlphaRecon.coh.mat']);
            fname                           = [flist(1).folder filesep flist(1).name];
            fprintf('loading %s\n',fname);
            load(fname); network_bsl = network_full.degrees; clear network_full;
            
            flist                            = dir(['I:\bil\source\' subjectName '.*Hz.'  ...
                list_window{ntime} '.' list_cond{ncond} '.AlphaRecon.coh.mat']);
            fname                           = [flist(1).folder filesep flist(1).name];
            fprintf('loading %s\n',fname);
            load(fname); network_act = network_full.degrees; clear network_full;
            
            source                      	= [];
            source.pos                   	= template_grid.pos;
            source.dim                  	= template_grid.dim;
            source.inside                   = template_grid.inside;
            source.pow                    	= (network_act - network_bsl) ./ network_bsl;
            
            if length(source.pow) == length(source.pos)
                alldata{nsuj,ntime,ncond}	= source; clear source flist fname network_full
            else
                error('fuck');
            end
