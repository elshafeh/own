function new_data = h_nback_stack_two(data)
...
trial_vctor                             = 1:3:length(data.trial);

for nt = 1:length(trial_vctor)-1
    ...
    lu                                  = lu+1;
    ix1                                 = find(round(data.time{lu},3) == round(0,3));
    ix2                                 = length(data.time{lu});% find(round(data.time{lu},3) == round(2,3));
    data_stim3                          = data.trial{lu}(:,ix1:ix2);
    tmp_info                            = [tmp_info 1003 data.trialinfo(lu,:)];
