...
for sb = 1:length(suj_list)
    ...
    if ~strcmp(suj(1:2),'fp') % avoid patients :)
        ...
        pkg.time_of_interest                = [0.08 -0.12];
        pkg.time_window                     = [0.07 0.07];
        pkg.covariance_window               = [-0.2 0.7];
        
        pkg.suj                             = suj;
        pkg.cond_main                       = [list_cond_main{:}];
        
        %         load(['../data/' suj '/field/' suj '.CnD.lcmvCommonFilter.concatenate_window.Covariance.m250m50p650p850p850p1050p1000p1200ms.mat']);
        %         spatial_filter{2} = spatialfilter;
        %         load(['../data/' suj '/field/' suj '.CnD.lcmvCommonFilter.large_window.Covariance.m300p1250ms.mat']);
        %         spatial_filter{1} = spatialfilter;        
        ...
        pkg.cond_ix_cue                     = {0:2,0,[1 2]};
        pkg.cond_ix_dis                     = {1:2,1:2,1:2};
        pkg.cond_ix_tar                     = {1:4,1:4,1:4};
        ...
        for nfilt = 1:length(spatial_filter)
            ...
            for ndata = 1:length(data_carrier)
                for ncue = 1:length(pkg.cond_ix_sub)
                    ...
                    pkg.ext_name                        = ['../data/' suj '/field/' suj '.' pkg.cond_ix_sub{ncue} list_cond_main{ndata} '.' list_filter_name{nfilt}];
