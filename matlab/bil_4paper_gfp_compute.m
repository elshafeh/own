...
for nsuj = 1:length(suj_list)
    ...
    if ~strcmp(subjectName,'sub004') && ~strcmp(subjectName,'sub023') % one trigger was missing for these trials
        ...
        list_window                             = {'preCue1' 'preCue2' 'preGab1' 'preGab2'};
        
        for nlock = 1:length(list_lock)
            
            fname                            	= [subject_folder subjectName '.' list_lock{nlock} '.lock.broadband.centered.mat'];
            fprintf('loading %s\n',fname);
            load(fname);
            ...
            for nband = 1:length(list_band)
                ...
                chk                             = dir([project_dir 'data/' subjectName '/tf/' subjectName '.allbandbinning.' list_band{nband} '.band.' list_window{nlock} '.window.index.mat']);
                ...
                if length(chk) == 1
                    ...
                    for nbin = [1 5]
                        ...
                        fname_out           	= [project_dir 'data/' subjectName '/erf/' subjectName '.' list_lock{nlock} '.lock.binning.' list_band{nband} '.band.' list_window{nlock} '.window'];
