...
for nsuj  = 1:length(list_suj)
    ...
    for ncond = 1:length(list_cond)
        ...
        my_freq                                 = 5;
        flist                                   = [dir(['J:/temp/nback/data/coef_mtm/sub' num2str(list_suj(nsuj)) '.sess*.5Hz.' list_cond{ncond} '.target.4stim.coef.lcmv.mat'])];
...
for ntest = 2
    ...
    for nview = 1:length(list_view)
        ...
        fout = [fout 'category.mtm.contrast.' list_test_name{ntest} '.' list_view_name{nview} '.png']; 
        saveas(gcf,fout);
        %         close all;
