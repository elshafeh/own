...
for nsuj = 1:length(suj_list)
    ...
    for ncond = 1:length(list_cond)
        ...
        ext_ext                                 = 'GammaCohImag'; % AlphaCohImag BetaCohImag ThetaCohImag GammaCohImag
        
        % -- load in baseline data
        flist                                   = dir(['I:/bil/source/' subjectName '.*Hz.m600m200'  ...
            '.' list_cond{ncond} '.' ext_ext '.mat']);
        fname                                   = [flist(1).folder filesep flist(1).name];
        fprintf('loading %s\n',fname);
        load(fname); coh_bsl = source_conn; clear source_conn;
        
        coh_bsl                                 = nanmean(coh_bsl(max_vox,:),1);
        
        for ntime = 1:length(list_window)
            
            % -- load in window of interest
            flist                               = dir(['I:/bil/source/' subjectName '.*Hz.'  ...
                list_window{ntime} '.' list_cond{ncond} '.' ext_ext '.mat']);
            fname                               = [flist(1).folder filesep flist(1).name];
            fprintf('loading %s\n',fname);
            load(fname); coh_act = source_conn; clear source_conn;

            % -- average across voxels with highest ERF grating response
            ...
            alldata{nsuj,ntime,ncond}           = source; clear source flist fname source_conn
            
        end
    end
end

keep alldata list_* ext_ext
...
keep alldata list_* stat min_p ext_ext;
close all;
...
for ntime = 1:size(alldata,2)
    ...
    if min_p(ntime) < plimit
        ...
        for nview = [1 2 3]
            ...
            material dull
            title([ext_ext ' pre v retro ' list_window{ntime} ' p = ' num2str(round(min_p(ntime),3))]);
