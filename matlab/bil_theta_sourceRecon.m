...
for nsuj = 1:length(suj_list)
    ...
    for nf = 1:length(list_freq)
        ...
        pkg_source(1).time_win                          = [4.5 5.5]; % [-1.2 -0.2; 1 2; 2 3; 3 4];
        
        %         % correct v incorrect
        %         pkg_source(2).indx_trials{1}                    = find(trialinfo(:,3) == 1);
        %         pkg_source(2).indx_trials{2}                    = find(trialinfo(:,3) == 2);
        %         pkg_source(2).name_cond                         = {'correct','incorrect'};
        %         pkg_source(2).time_win                          = [-0.6 -0.2; 2.8 3.2];
        %
        %         % pre v retro
        ...
        %         pkg_source(3).time_win                          = [-1.2 -0.2; 3.65 4.65; -0.6 -0.2; 3.2 3.6];
            ...
            for ncond = 1:length(pkg_source(npkg).name_cond)
                for ntime = 1:size(pkg_source(npkg).time_win,1)
                    ...
                    fname_out                           = [start_dir '3015079.01/data/' subjectName '/source/' subjectName '.' num2str(f1) 't' num2str(f2) 'Hz.'  ... 
                        ext_time_source '.' pkg_source(npkg).name_cond{ncond} '.ThetaReconDics.mat'];
                    fprintf('saving %s\n',fname_out);
