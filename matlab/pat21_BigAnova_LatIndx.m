...
for sb = 1:size(source_avg,1)
    ...
    for cnd = 1:3
        ...
        for chn = 1:2:10
            ...
            ix = ix+1;
            act_l = squeeze(source_avg(sb,cnd,chn,:,:));
            act_r = squeeze(source_avg(sb,cnd,chn+1,:,:));
            
            indx  = (act_r-act_l) ./ (mean(cat(3,act_l,act_r),3));
            
            new_source_avg(sb,cnd,ix,:,:) = indx;
            nw_chn_list{ix} = template.label{chn}(1:end-1);
...
template.label = nw_chn_list ; source_avg = new_source_avg ;

clearvars -except template source_avg fin_ext

 % Create the text file

time_list = [-0.6 -0.4 -0.2 0:0.1:0.9];

fOUT = ['../txt' fin_ext '.m' num2str(abs(time_list(1)*1000)) '.p' num2str(abs(time_list(end)*1000)) '.LatIndex.txt'] ; clear t_* ;
fid  = fopen(fOUT,'W+');

fprintf(fid,'%4s\t%4s\t%7s\t%5s\t%5s\t%5s\n','SUB','COND','CHAN','FREQ','TIME','LIndx');
        ...
        for chn = 1:size(source_avg,3)
            ...
            for f = 1:size(source_avg,4)
                ...
                for t = 1:length(time_list)
                    ...
                    y = find(round(template.time,2) == round(time_list(t)+0.2,2));
