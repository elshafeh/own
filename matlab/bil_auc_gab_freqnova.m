...
for nsuj = 1:length(suj_list)
    ...
    for nfreq = 1:length(frequency_list)
        ...
        for ndeco = 1:length(decoding_list)
            ...
            flist                   = dir([dir_data subjectName '.*.lock.' frequency_list{nfreq} ...
        ...
        if fs > 30
            ...
            orig_time_axis          = load([dir_data 'sub001.2ndgab.lock.theta.minus1f.centered.decodinggabor.orientation.correct.bsl.auc.mat'],'time_axis');
            ...
            t2                      = find(round(avg.time,2) ==round(0.95,2));
...
for nchan = 1:length(stat.label)
    ...
    vline(0,'--k');
    xticks([0 0.5 1]);
    xticklabels({'Gabor Onset' '0.5' '1'});
    hline(0.5,'--k');
    
    set(gca,'FontSize',16,'FontName', 'Calibri','FontWeight','normal');
    
    zlimit                    	= [0.46 0.8;0.48 0.6];
