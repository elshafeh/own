...
for sb = 51:51 %1:length(suj_list)
    ...
    if length(suj_list(sb).name) > 2 && length(suj_list(sb).name) < 5
        ...
        for nbloc = 1:length(blocksArray)
            ...
            dir_cop_ds =  [direc_ds suj '.pat2.b' num2str(str2double(blocksArray{nbloc})) '.ds'];
            dir_trd_ds =  [direc_ds suj '.pat2.b' num2str(str2double(blocksArray{nbloc})) '.thrid_order.ds'];
        ...
        writetable(headerResults,['../data/' suj '/res/' suj '.header.before.after.csv']);
