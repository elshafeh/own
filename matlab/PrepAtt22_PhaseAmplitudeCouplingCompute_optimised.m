...
for sb = 1:length(suj_list)
    ...
    ext_essai                   = 'prep21.maxAVMsepVoxels';
    ...
    ext_freq                    = ['low.' num2str(pha_freq_vec(1)) 't' num2str(pha_freq_vec(2)) '.high5step.' num2str(amp_freq_vec(1)) 't' num2str(amp_freq_vec(2))];
    
    [mpac,mpac_norm,mpac_surr,mpac_std]  = separate_calc_MI_optimised(original_virtsens_pha,original_virtsens_amp, ...
        list_toi,pha_freq_vec,amp_freq_vec,'no','yes',list_method,pha_step,amp_step,chan_list,trial_index,'yes');
        ...
        for ntime = 1:length(list_toi)
            for nmethod = 1:length(list_method)
                for nchan = 1:length(chan_list)
                    ...
                    fname_ext1      = [dir_data suj '.' list_cue{ncue} '.' ext_essai '.sepFreq'];
