...
for nsuj = 1:length(suj_list)
    ...
    for nback = [1 2]
        ...
        %         sub_info           	= trialinfo(trialinfo(:,1) == nback+4 & trialinfo(:,2) == 1,[4 5 6]);
        
        % choose allstim
        sub_info           	= trialinfo(trialinfo(:,1) == nback+4,[4 5 6]); %trialinfo(:,[4 5 6]); %
    ...
    for nrt = [1 2]
        ...
        fname_out         	= ['~/Dropbox/project_me/data/nback/bin_index/sub' num2str(suj_list(nsuj)) '.' list_rt{nrt} '.newindex.allstim.mat'];
        fprintf('saving %s\n',fname_out);
