...
for nsuj = 1:length(suj_list)
    ...
    list_band                       = 'beta';% alpha beta;
    list_lock                       = {'first' 'target'};
    
    if strcmp(list_band,'alpha')
        nband                       = 1;
        ...
        for nback = 1:length(list_cond)
            ...
            for nfreq = 1:length(list_freq)
                file_list         	= dir(['J:/nback/sens_level_auc/cond/sub' num2str(suj_list(nsuj)) '.decoding.' ...
                    list_cond{nback} '.agaisnt.all.' num2str(list_freq(nfreq)) 'Hz.lockedon.' list_lock{nlock} '.dwn70.bsl.excl.auc.mat']);
            ...
            list_final{i}        	= ['B' list_cond{nback}(1) ' ' list_lock{nlock}(1:2)];
            fprintf('\n');
...
if isfield(stat,'posclusters')
    for ncluster = 1:length(stat.posclusters)
        ...
        if length(unique(vct)) > 1
            ...
            title({['1 v 2 = ' num2str(round(p1,3))], ...
                ['1 v 3 = ' num2str(round(p2,3))],...
                ['1 v 4 = ' num2str(round(p3,3))], ...
                ['2 v 3 = ' num2str(round(p4,3))],...
                ['2 v 4 = ' num2str(round(p5,3))],...
                ['3 v 4 = ' num2str(round(p6,3))]});
