...
for nsuj = 1:length(suj_list)
    ...
    for nsession = 1:2
        for nback = [1 2 3]
            ...
            chk                                                     = dir(['J:/temp/nback/data/source/alpha/' subjectname '.session' num2str(nsession) '.' num2str(nback-1) 'back.*.dics.mat']);
            
            if ~isempty(chk)
                
                for ntime = 1:length(list_time)
                    
                    fname                                           = dir(['J:/temp/nback/data/source/alpha/' subjectname '.session' num2str(nsession) '.' num2str(nback-1) 'back.*.' list_time{ntime} '.dics.mat']);
                    fprintf('loading %s\n',fname.name);
