...
for nsuj  = 1:length(list_suj)
    ...
    list_cond                	= {'0back.target.4stim','1back.target.4stim','2back.target.4stim'};
...
load ../data/stat/nbk.mtm.center.4stim.mat
...
for ntest = 1:3
    for nfreq = 1:2
        ...
        for nview = 1:size(list_view,1)
            ...
            list_test_name      = {'0B vs 1B','0B v 2B','1B v 2B'};
            title([list_test_name{ntest} ' ' list_freq{nfreq}]);
            
            saveas(gcf,['../figures/nback/coef_mtm/stim/coef.' list_test_name{ntest} '.' list_freq{nfreq} ...
