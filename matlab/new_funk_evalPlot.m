...
function new_funk_evalPlot(summary)
...
nw_summary  = [summary(:,13) summary(:,6) rpt rpt rpt rpt summary(:,11) summary(:,14) summary(:,15) summary(:,12) summary(:,10)];

summary     = nw_summary;

% ---- FOR CORRECT, MISSES AND FALSE ALARMS --- %

list_marker = {'h','d','d','h','o','o'};
list_size   = [0 1 0;0 .9 0;0 .5 0;0 .5 1;0 .5 .8;0 .3 .7];
list_color  = [0 1 0;0 .9 0;0 .5 0;0 .5 1;0 .5 .8;0 .3 .7];
...
for h_cue = 1:2
    for h_dis = 1:3
        ...
        to_plot(summary(:,1)==h_cue & summary(:,11)~=-1) = summary(summary(:,1)==h_cue & summary(:,11)~=-1, 7);
        to_plot(summary(:,2)~=h_dis-1) = NaN;
        plot(to_plot, list_marker{h_i}, 'Color', list_size(h_i,:), 'MarkerSize', 5, 'MarkerFaceColor', list_color(h_i,:))
        hold on
...
for h_cue = 1:2
    for h_dis = 1:3
        ...
        to_plot(summary(:,1)==h_cue & summary(:,11)==-1) = summary(summary(:,1)==h_cue & summary(:,11)==-1, 7);
        to_plot(summary(:,2)~=h_dis-1) = NaN;
        plot(to_plot, 'x', 'Color', 'r', 'MarkerSize', 5, 'MarkerFaceColor', list_color(h_i,:))
        hold on
...
plot([0 length(to_plot)], [3350 3350], 'Color', [1 .7 0])
text(0.5, lowerRTlimit-60, 'Following Cue', 'Color', [1 .7 0]);
