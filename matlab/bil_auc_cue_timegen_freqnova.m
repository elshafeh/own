...
for nsuj = 1:length(suj_list)
    ...
    for nfreq = 1:length(frequency_list)
        ...
        list_time                 	= [0.1 0.5; 3.1 3.5]; % [0.1 0.8; 3 4.5]; % 
        tmp                       	= [];
        
        for nchan = 1:length(decoding_list)
            t1                      = find(round(time_axis,2) == round(list_time(nchan,1),2));
            t2                      = find(round(time_axis,2) == round(list_time(nchan,2),2));
            tmp(nchan,:)            = squeeze(mean(data(nchan,t1:t2,:),2)); clear t1 t2
        end
