...
for nsuj = 1:length(suj_list)
    ...
    for nband = 1:length(list_band)
        for nstim = 1:length(list_stim)
            ...
            file_list               = dir(['J:/nback/sens_level_auc/kia/sub' num2str(suj_list(nsuj)) '.sess*.*back' ...
                '.' list_band{nband} '.peak.centered.lockedon.' list_stim{nstim} '.decoding.stim*.agaisnt.all.bsl.excl.auc.mat']);
            ...
            list_cond{i} 	= [list_stim{nstim} ' ' list_band{nband}(1:2)];
            fprintf('\n');
        end
    end
end
