...
for nsuj = 1:length(suj_list)
    ...
    for nbin    = 1:5
        ...
        ext_source                          = 'AlphaDics';
        list_window                         = 'p4100p5100';
        flist                               = dir([project_dir 'data/' subjectName '/source/' subjectName '.*Hz.' list_window '.itcbin' num2str(nbin) '.' ext_source '.mat']);
        
        for nf  = 1:length(flist)
            flg                             = strfind(flist(nf).name,'-1');
            if isempty(flg)
