...
% ICA_table = readtable('../documents/PrepAtt22_ICA_fenetres.xlsx');
% suj_list  = ICA_table.suj;

ICA_table = readtable('../documents/redo_ICA_timepoints','Delimiter',';');
...
for sb = 1:length(suj_list)
    ...
    chk_suj     = strsplit(eeg_file_list{4,2},'.');
    chk_suj     = chk_suj{1};
    
    if strcmp(chk_suj,suj)
        
        [m_data,m_events,~,~,~,s_nb_channel_all,v_label_all,~,~,~,~] = eeg2mat(dirIN,200,210,'all');
