...
for ngroup = 1:length(suj_group)
    ...
    for sb = 1:length(suj_list)
        ...
        cond_main           = {'CnDV','CnDN'}; % {'CnDR','CnDNR','CnDL','CnDNL'};
        
        for nfreq = 1:length(lst_freq)
            for ntime = 1:length(lst_time)
                for cnd_cue = 1:length(cond_main)
                    
                    fname = ['../data/pat22_targetGamma/' suj '.' cond_main{cnd_cue} '.' lst_freq{nfreq} '.' lst_bsl '.' ext_comp];
                    fprintf('Loading %50s\n',fname);
                    load(fname);
                    
                    bsl_source            = source; clear source
                    
                    fname = ['../data/pat22_targetGamma/' suj '.' cond_main{cnd_cue} '.' lst_freq{nfreq} '.' lst_time{ntime} '.' ext_comp];
                    fprintf('Loading %50s\n',fname);
                    load(fname);
