...
for sb = 1:length(suj_list)
    ...
    if ~strcmp(suj(1:2),'fp') % avoid patients :)
        ...
        pkg.time_of_interest                = [0.5 -0.75];
        pkg.time_window                     = [0.7 0.7];
        pkg.covariance_window               = [-0.8 1.3];
        pkg.lambda                          = '5%';
        ...
        pkg.cond_ix_sub                     = {'','V','N','L','R'};
        
        pkg.cond_ix_cue                     = {0:2,[1 2],0,1,2};
        pkg.cond_ix_dis                     = {0,0,0,0,0,};
        pkg.cond_ix_tar                     = {1:4,1:4,1:4,1:4,1:4};
