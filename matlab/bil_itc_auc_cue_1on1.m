    ...
    for nfreq = 1:length(list_freq)
        for nfeat = 1:length(list_feat)
            for nbin = 1:length(list_bin)
                ...
                fname                  	= 	[dir_data subjectName '.decodingcue.1stcue.lock.' list_freq{nfreq} '.centered.' list_bin{nbin} '.' list_feat{nfeat} '.all.bsl.auc.mat'];
                fprintf('loading %s\n',fname);
                load(fname);
