function h_prep21_pccSeparate(suj,n_prt,data_in,list_time,time_win,trilili,f_focus,tap)
...
for t = 1:length(list_time)
    ...
    load(['/Volumes/Pat22Backup/meg21_fieldtrip_data_backup/data/all_data/' suj '.VolGrid.0.5cm.mat']);
    
    ext_freq                = [num2str(f_focus-tap) 't' num2str(f_focus+tap) 'Hz'];
    
    ext_com                 = '.CnD.m800p2000.5t15Hz.NewCommonFilter' ;
    FnameFilterIn           = ['/Volumes/PAT_MEG2/Fieldtripping/data/all_data/' suj '.pt' num2str(n_prt) ext_com '.mat'];
    ...
    save(['/Volumes/PAT_MEG2/Fieldtripping/data/all_data/' f_name_source '.mat'],'source','-v7.3');
