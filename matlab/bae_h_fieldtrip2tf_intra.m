function h_fieldtrip2tf_intra(tf_file_name,tfavg,Fs)
...
mat2tf_intra(tf_file_name, 1, 1, 1, 1,tfavg.powspctrm(:,:,:), v_Elec, tfavg.label, ...
    s_Nb_Sample_PreStim, round(nw_Fs), 1, tfavg.freq, repmat(7,length(tfavg.freq),1), ...
    1, round(Fs/nw_Fs), ...
    ones(1,length(tfavg.label)),0, 0, 0);