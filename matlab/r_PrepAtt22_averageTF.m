...
for ev = 1:length(event_names) %bp, cue, dis, fdis, target
    ...
    for cat = 1:length(event_list)
        ...
        if exist(['../data/' list_ctl{1} '/tf/' event_names{ev} '/' list_ctl{1} '.pat22.' event_list{cat} ext]) == 2;
            
            cd(['../data/avg_mig/tf/' event_names{ev}])
            
            system(['rm ctl.pat22.' event_list{cat} ext ])
            system(['rm mig.pat22.' event_list{cat} ext ])
            system('rm TF.list')
            
            %%ctl
            FicName     = 'TF.list';
            fid         = fopen(FicName,'w+');
            tffileOUT   = ['ctl.pat22.' event_list{cat} ext];
            ...
            for sb = 1:length(list_ctl)
                suj      = list_ctl{sb};
                tffileIN = [ '../../../' suj '/tf/' event_names{ev} '/' suj '.pat22.' event_list{cat} ext ];
                fprintf(fid,'%s\n',tffileIN);
            end
            ...
            FicName     = 'TF.list';
            fid         = fopen(FicName,'w+');
            tffileOUT   = ['mig.pat22.' event_list{cat} ext];
