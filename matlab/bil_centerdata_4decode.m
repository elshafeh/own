...
for nsuj = 1:length(suj_list)
    ...
    for nlock = 1:length(data_axial)
        ...
        for nfoi = 1:length(list_foi)
            ...
            fname_out               = ['F:/bil/preproc/' subjectName '.' list_lock{nlock} '.lock.' list_name{nfoi} '.centered.mat'];
            fprintf('Saving %s\n',fname_out);
            save(fname_out,'data','-v7.3');
            
            fname_out               = ['F:/bil/preproc/' subjectName '.' list_lock{nlock} '.lock.' list_name{nfoi} '.centered.trialinfo.mat'];
            fprintf('Saving %s\n',fname_out);
