...
keep suj_list allpeaks *_dir; clc;

for nsuj = 1:length(suj_list)
    
    subjectName                                         = suj_list{nsuj};
    
    fname                                               = ['I:/bil/head/' subjectName '.volgridLead.1cm.withNas.mat'];
    fprintf('loading %s\n',fname);
    load(fname);
    ...
    list_freq                                           = [4 80];
    list_smooth                                         = [1 20];
    list_ext                                            = {'ThetaCohImag' 'GammaCohImag'};
    
    %     list_freq                                           = round(allpeaks(nsuj,:));
    %     list_smooth                                         = [1 20];
    %     list_ext                                            = {'AlphaCohImag' 'BetaCohImag'};
    ...
    for nf = 1:length(list_freq)
        ...
        pkg_source(1).time_win                          = [-0.6 -0.2; 0.5 1.5; 2 3; 3.5 4.5; 5 6];
        ...
        for npkg = 1:length(pkg_source)
            for ncond = 1:length(pkg_source(npkg).name_cond)
                for ntime = 1:size(pkg_source(npkg).time_win,1)
                    ...
                    source_conn                         = source_conn.cohspctrm;
                    ...
                    fname_out                           = ['I:\bil\source\' subjectName '.' num2str(f1) 't' num2str(f2) 'Hz.'  ... 
                        ext_time_source '.' pkg_source(npkg).name_cond{ncond} '.' list_ext{nf} '.mat'];
                    fprintf('saving %s\n',fname_out);
                    save(fname_out,'source_conn','-v7.3'); 
