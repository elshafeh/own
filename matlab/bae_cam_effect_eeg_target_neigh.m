...
for ngroup = 1:size(stat,1)
    for ntest = 1:size(stat,2)
        for nchan = 1:length(stat{ngroup,ntest}.label)
            ...
            h_plotSingleERFstat_selectChannel(cfg,stat{ngroup,ntest}, ...
                gavg_data{ngroup,ix_test(ntest,1)}, ...
                gavg_data{ngroup,ix_test(ntest,2)});
...
% save 'cluster_based_permutations_young_target_eeg_channels_neigh.mat'