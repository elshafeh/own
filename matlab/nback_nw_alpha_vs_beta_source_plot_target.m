...
for nsuj = 1:length(suj_list)
    ...
    list_time                                            	= {'m900m200','p300p1000'};
    
    list_cond                                               = {'1back.target.alpha' '2back.target.alpha' '1back.target.beta' '2back.target.beta'};
...
for ncond = 1:size(list_test,1)
    ...
    material dull
    title(test_title);
    saveas(gcf,['D:\Dropbox\project_me\figures_me\nback\post-kia\source\univar\alpha v beta\target\test' num2str(ncond) '.left.png']);
    ...
    material dull
    title(test_title);
    saveas(gcf,['D:\Dropbox\project_me\figures_me\nback\post-kia\source\univar\alpha v beta\target\test' num2str(ncond) '.right.png']);
