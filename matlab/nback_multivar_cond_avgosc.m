...
for nsuj = 1:length(suj_list)
    ...
    for nlock = 1:length(list_lock)
        for nback = 1:length(list_nback)
            ...
            for nband = 1:2
                list_freq       	= [list_freq round(allpeaks(nsuj,nband)-nband : allpeaks(nsuj,nband)+nband)];
            end
            ...
            for nfreq = 1:length(list_freq)
                ...
                file_list         	= dir([dir_data suj_name '.decoding.' list_nback{nback} '.agaisnt.all.' num2str(list_freq(nfreq)) 'Hz.lockedon.' list_lock{nlock} fin_ext]);
            ...
            list_cond{i}        	= [list_lock{nlock} ' '  list_nback{nback}(1:2)];
            fprintf('\n');
        end
    end
end
