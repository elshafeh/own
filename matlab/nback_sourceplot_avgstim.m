...
for nsuj = 1:length(suj_list)
    ...
    list_time                                       	= {'m880m200' 'p320p1000'};
    
    list_cond                                           = {{'1back' 'alpha'} {'2back' 'alpha'} {'1back' 'beta'} {'2back' 'beta'}};
    
    load('../data/stock/template_grid_0.5cm.mat');
    
    for ncond = 1:length(list_cond)
        
        list_temp                                       = {'first' 'target'};
        
        for ntemp = 1:2
            for ntime = 1:2
                fname                               	= dir(['J:/nback/source/combined/' subjectname '.' list_cond{ncond}{1} '.' list_temp{ntemp} ... 
                   '.' list_cond{ncond}{2} '.*.' list_time{ntime} '.dicsCombined.mat']);
                fprintf('loading %s\n',fname.name);
...
close all;
list_test                                   = [1 3; 4 3; 3 1; 4 2];
list_z                                      = [0.04 0.04 0.1 0.1];

for ntest = 1:2 %size(list_test,1)
    ...
    test_title                              = {[list_cond{list_test(ntest,1)}{1} '.' list_cond{list_test(ntest,1)}{2} ' vs'], ...
        [list_cond{list_test(ntest,2)}{1} '.' list_cond{list_test(ntest,2)}{2}]};
