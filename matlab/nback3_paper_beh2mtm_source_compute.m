...
for nsuj = 1:length(suj_list)
    ...
    [index_rt]                          = func_beh2mtm_split(data,'target');
    ...
    vct_time                            = [-0.3 0.6; 0.7 1]; % [0.45 1.75]; % [0.6 2; -0.4 0.65; 0.45 0.75];
    ...
    for ntime = 1:size(vct_time,1)
        ...
        for nfreq = 1:length(vct_freq)
            ...
            com_filter                  = nbk_common_filter(data,leadfield,vol,[t1 t2],allpeaks(nsuj,1),1); % alpha/beta [+/- 1Hz]
                        
            % separate for each RT bin [median]
            for nrt = [1 2]
                
                cfg                  	= [];
                cfg.trials            	= index_rt{nrt};
                ...
                fname_out               = ['~/Dropbox/project_me/data/nback/source/rt/' subjectname '.' list_rt{nrt} '.allback.'];
                fname_out               = [fname_out 'target.' list_freq{nfreq} '.' ext_name  '.dicsCombined.mat'];
                fprintf('\nsaving %s\n',fname_out);
