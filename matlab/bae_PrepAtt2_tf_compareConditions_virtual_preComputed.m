...
for ngroup = 1:length(suj_group)
    ...
    for sb = 1:length(suj_list)
        ...
        ext_name2               = 'BroadAVM.waveletPOW.1t20Hz.m3000p3000.AvgTrialsMinEvoked80Slct';
        list_ix_cue             = {'R','L','NR','NL','N'};
        
        for ncue = 1:length(list_ix_cue)
            
            
            fname_in                = ['../data/' suj '/field/' suj '.' list_ix_cue{ncue} cond_main '.' ext_name2 '.mat'];
            fprintf('\nLoading %50s \n',fname_in);
            load(fname_in)
...
for ngroup = 1:length(allsuj_data)
    ...
    list_compare            = [6 7; 8 9]; %[1 3; 2 4;1 2; 3 4; 1 5; 2 5];
...
for ngroup = 1:size(stat,1)
    for ntest = 1:size(stat,2)
        for nchan = 1:length(stat{ngroup,ntest}.label)
            ...
            if min(min(min(s2plot.prob))) < plimit
                ...
                title([lstgroup{ngroup} ' ' s2plot.label{:} ' ' list_test{ntest} ' ' num2str(min_p(ngroup,ntest))]);
