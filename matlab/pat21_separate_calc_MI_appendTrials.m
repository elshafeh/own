...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [MI_matrix_raw,MI_matrix_surr,MI_raw_surr] = separate_calc_MI_appendTrials(suj,original_virtsens_pha,original_virtsens_amp,toi,phase,amp,diag,surrogates,approach,step_phase,step_amp,varargin)
...
MI_raw_surr     = zeros(amp_length,phase_length);
clear phase_length amp_length

row1            = 1;
row2            = 1;

ntest          = length(phase(1):2:phase(2)) * length(amp(1):5:amp(2));

iphase         = 0;
iampl          = 0;

hesham         = 0;
% h_wait         = waitbar(0,['Phase Filtering for ' upper(approach) ' ' suj]);

for phase_freq = phase(1):step_phase:phase(2) % steps of 2 Hz
    
    
    hesham = hesham + 1;
    %     waitbar(hesham/length(phase(1):step_phase:phase(2)))
    ...
    [virtsens_phase{iphase}]    = ft_preprocessing(cfg, original_virtsens_pha);
    
    
    cfg                         = [];
    cfg.toilim                  = toi; %specfied in function calls
    cfg.showcallinfo            = 'no';
    virtsens_phase_toi{iphase}  = ft_redefinetrial(cfg,virtsens_phase{iphase});
    
    data_trial{1}               = [];
    
    for ij = 1:length(virtsens_phase_toi{iphase}.trial)
        data_trial{1} = [data_trial{1} virtsens_phase_toi{iphase}.trial{ij}];
    end
    
    nw.label                    = virtsens_phase_toi{iphase}.label;
    nw.trial                    = data_trial;
    nw.time                     = {1:length(data_trial{1})};
    nw.fsample                  = virtsens_phase_toi{iphase}.fsample;
    nw.trialinfo                = 1;

    virtsens_phase_toi{iphase}  = nw;
...
hesham         = 0;
% h_wait         = waitbar(0,['Amplitude Filtering for ' upper(approach) ' ' suj]);
...
for amp_freq = amp(1):step_amp:amp(2) % steps of 5 Hz
    ...
    cfg.toilim                  = toi; %specfied in function calls
    cfg.showcallinfo            = 'no';
    virtsens_amp_toi{iampl}     = ft_redefinetrial(cfg,virtsens_amp{iampl});
    
    data_trial{1}               = [];
    
    for ij = 1:length(virtsens_amp_toi{iampl}.trial)
        data_trial{1} = [data_trial{1} virtsens_amp_toi{iampl}.trial{ij}];
    end
    
    nw.label                    = virtsens_amp_toi{iampl}.label;
    nw.trial                    = data_trial;
    nw.time                     = {1:length(data_trial{1})};
    nw.fsample                  = virtsens_amp_toi{iampl}.fsample;
    nw.trialinfo                = 1;
    virtsens_amp_toi{iampl}     = nw;
...
% close(h_wait);
hesham         = 0;
% h_wait         = waitbar(0,['Computing PAC using ' upper(approach) ' for ' suj]);

for han_phase_i = 1:iphase
    for han_ampl_i = 1:iampl
        
        %         waitbar(hesham/ntest)
        hesham = hesham + 1;
        ...
        for trial_num = 1
            ...
            Phase       = virtsens_phase_toi{han_phase_i}.trial{1, trial_num}; % getting the phase
            Amp         = virtsens_amp_toi{han_ampl_i}.trial{1, trial_num}; % getting the amplitude envelope
        ...
        if strcmp(surrogates, 'yes')
            ...
            for surr = 1:200
                ...
                trial_num = randperm(length(virtsens_phase_toi{han_phase_i}.trialinfo),2);
                
                % Extract phase and amp info using hilbert transform
                % for different trials & shuffle phase
                Phase=virtsens_phase_toi{han_phase_i}.trial{1, trial_num(1)}(randperm(length(virtsens_phase_toi{han_phase_i}.trial{1,trial_num(1)}))); % getting the phase
                Amp = virtsens_amp_toi{han_ampl_i}.trial{1,trial_num(2)};
            ...
            MI_raw_surr(row1,row2)      = mean(MI_surr);
        ...
        (fprintf('Phase: %d Amplitude: %d  MI: %d\n',phase_freq,amp_freq,MI_raw));
