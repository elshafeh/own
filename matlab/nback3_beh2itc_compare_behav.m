...
for nsuj = 1:length(suj_list)
    ...
    for ncond = [1 2]
        ...
        flist                   = [dir_data 'sub' num2str(suj_list(nsuj)) '.allback.' list_stim];
        flist               	= [flist '.' list_cond{ncond} '.itc.withevoked.mat'];
        flist                   = dir(flist);
        ...
        for nband = [1 2]
            ...
            % baseline over time
            t1                  = nearest(phase_lock.time,-0.7);
            t2                  = nearest(phase_lock.time,-0.5);
