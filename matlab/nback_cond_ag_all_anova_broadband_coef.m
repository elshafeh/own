...
for nsuj  = 1:length(list_suj)
    ...
    for ncond = 1:length(list_cond)
        ...
        flist                                   = dir(['J:/temp/nback/data/source/coef/sub' num2str(list_suj(nsuj)) '.decoding.' list_cond{ncond} ... 
            '.agaisnt.all.lockedon.target.dwn70.bsl.excl.coef.combinedlead.lcmv.mat']);
...
load ../data/stat/anova/nback.cond.ag.all.broadband.mat

zlist                   = [0.05 0.05 0.05];
...
for ncluster = 1:length(cluster_list)
    for ntest = 1:size(list_test,1)
        ...
        for nview = 1:length(list_view)
            ...
            fout = [fout 'cond.ag.all.broadband.cluster' num2str(ncluster) '.' list_test_name{ntest} '.' list_view_name{nview} '.png'];
            saveas(gcf,fout);
