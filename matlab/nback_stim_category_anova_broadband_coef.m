...
for nsuj  = 1:length(list_suj)
    ...
    for ncond = 1:length(list_cond)
        ...
        sub_carr                              	= [];
        flist                                   = dir(['J:/temp/nback/data/source/coef/sub' num2str(list_suj(nsuj)) '.sess*.' list_cond{ncond} '.dwn70.target.deocdingStim.coef.lcmv.mat']);
...
load ../data/stat/anova/nback.stim.category.broadband.mat
...
for ncluster = 1:length(cluster_list)
    for ntest = 1:size(list_test,1)
        ...
        for nt = 1:length(tm_points)
            tim_indx = [tim_indx; find(time_axis == tm_points(nt))];
        end
        ...
        for nview = 1:length(list_view)
            ...
            fout = 'D:/Dropbox/project_me/pub/Papers/postdoc/nback_manuscript/_figures/_new_prep/source_contrast/';
            fout = [fout 'category.contrast.broadband.cluster' num2str(ncluster) '.' list_test_name{ntest} '.' list_view_name{nview} '.png'];
            saveas(gcf,fout);
            %         close all;
