    ...
    for cf = 1:3
        for ix = 1:2
            ...
            for cp = 1:3
                ...
                src_carr{ix} = [src_carr{ix} source] ; clear source ;
        ...
        for ix = 1:2
            for cp = 1:5
                tmp{cp,ix} = nanmean(src_carr{ix}(:,sexyfive{sb,cp}),2);
            end
        end
        
        src_carr = tmp ; clear tmp ;

        for cp = 1:5
            data(cp,:) =  (src_carr{cp,2} - src_carr{cp,1}) ./ src_carr{cp,1} ; %src_carr{cp,2} ;
            rt(cp,1)   = mean(rt_all{sb}(sexyfive{sb,cp}));
        end
