...
for nsuj = 1:length(suj_list)
    ...
    for nband = 1:length(list_band)
        for nback = 1:length(list_nback)
            ...
            list_freq               = round(allpeaks(nsuj,nband)-list_width(nband) : allpeaks(nsuj,nband)+list_width(nband));
            pow                  	= [];
    ...
    list_cond                      	= {'b1-b2 alpha' 'b1-b2 beta'};
    newdata{nsuj,1}.avg             = (alldata{nsuj,1}.avg - alldata{nsuj,2}.avg);
    newdata{nsuj,2}.avg          	= (alldata{nsuj,3}.avg - alldata{nsuj,4}.avg);
    
    %     list_cond                      	= {'a-b b1' 'a-b b2'};
...
for ntest = 1:size(list_test,1)
    ...
    cfg.correctm                  	= 'bonferroni'; % 'cluster';
