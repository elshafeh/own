...
for nsuj = 1:length(suj_list)
    ...
    for nstim = [1 2]
        for nback = [1 2]
            ...
            list_cond{i}      	= ['B' num2str(nback) ' ' list_stim{nstim}(1:2)];
            
        end
    end
end

keep alldata list_cond test_band
...
ylim(round(lm));
yticks(round(lm));
vline(0,'-k');title(test_band);

cfg                             = [];
cfg.channel                     = list_chan;
cfg.time_limit                  = stat.time([1 end]);
cfg.color                       = 'rbgk'; % 'rgbmck';
cfg.z_limit                     = [-0.5 0.5];
cfg.linewidth                   = 10;
subplot(nrow,ncol,3);
...
for ncluster = 1:length(stat.posclusters)
    ...
    if length(unique(vct)) > 1
        ...
        for nsub = 1:size(alldata,1)
            for ncond = 1:size(alldata,2)
                ...
                for nc = 1:length(stat.label)
                    find_chan_in_data            	= [find_chan_in_data; find(strcmp(stat.label{nc},alldata{nsub,ncond}.label))];
                end
                
                vct_y               = alldata{nsub,ncond}.avg(find_chan_in_data,t1:t2);
                data_plot(nsub,ncond)       = mean(mean(vct .* vct_y));
        ...
        title({['b1 f vs b2 f = ' num2str(round(p1,3))], ...
            ['b1 f vs b1 ta = ' num2str(round(p2,3))],...
            ['b1 f vs b2 ta = ' num2str(round(p3,3))], ...
            ['b2 f vs b1 ta = ' num2str(round(p4,3))],...
            ['b2 f vs b2 ta = ' num2str(round(p5,3))],...
            ['b1 ta vs b2 ta= ' num2str(round(p6,3))]});
