function [sph, spow, phase_alltr, peaks_all] = ft_PAC_original(cfgTF,cfg,data_raw)
%% input
  % cfgTF       : paramters for the time frequency analysis
        % cfgTF.method (e.g. mtmconvol)
        % cfgTF.output (e.g. pow)
        % cfgTF.foi etc... (see fieldtrip tutorial: http://www.fieldtriptoolbox.org/tutorial/timefrequencyanalysis/)
...
% get the "filtered" signal (low frequency):selection of the window of interest in the frequency of interest for each trial (from tf data or from filtered data)
    if     strcmp(cfg.meth ,'TF')                                          
      ...
      %       hesham_tmp           = real(squeeze(data_phase.fourierspctrm(:,:,:,datawin)));
      %       for nhesham = 1:size(hesham_tmp,1)
      %           phase{nhesham}   = squeeze(hesham_tmp(nhesham,:,:));
      %       end
...
for itr                     = index'
    ...
    if numel(peaks{tr})>0
        for ip              = 1:numel(peaks{tr})
            ...
            if any(ismember(signal(datawin(troughs{tr})),signal(datawin(peaks{tr}(ip))-timetr))) && ...                 %((1/cfg.freq)/2-cfg.timewintr:cfgTF.toi(2)-cfgTF.toi(1):(1/cfg.freq)/2+cfg.timewintr)))  && ...
               any(ismember(signal(datawin(troughs{tr})),signal(datawin(peaks{tr}(ip))+ timetr)))                  %((1/cfg.freq)/2-cfg.timewintr:cfgTF.toi(2)-cfgTF.toi(1):(1/cfg.freq)/2+cfg.timewintr)))
