...
for nsuj  = 1:length(list_suj)
    ...
    list_cond                                   = {'alpha.peak.centered.*back.isfirst','alpha.peak.centered.*back.istarget', ...
        'beta.peak.centered.*back.isfirst','beta.peak.centered.*back.istarget'};
...
for ncluster = 1:length(cluster_list)
    for ntest = 1:size(list_test,1)
        ...
        for nview = 1:length(list_view)
            ...
            fout = ['D:/Dropbox/project_me/figures_me/nback/post-kia/source/multivar/nbk_avg/test' num2str(ntest) '.' list_view_name{nview} '.png'];
            saveas(gcf,fout);
