...
ext_fix     = 'istarget'; % isfirst istarget

for nsuj = 1:length(suj_list)
    
    suj_name                                    = ['sub' num2str(suj_list(nsuj))];
    list_lock                                   = {['alpha.peak.centered.' ext_fix],['beta.peak.centered.' ext_fix]};
    ...
    for nback = 1:length(list_cond)
        for nlock = 1:length(list_lock)
            ...
            flist                               = dir(['J:/nback/sens_level_auc/timegen/' suj_name '.sess*' ...
