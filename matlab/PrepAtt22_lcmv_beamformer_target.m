...
for sb = 1:length(suj_list)
    ...
    if ~strcmp(suj(1:2),'fp') % avoid patients :)
        ...
        pkg.time_of_interest                = [0.035 0.08 0.15 0.25];
        pkg.time_window                     = [0.035 0.07 0.05 0.25];
        
        tmp_win                             = -0.05 - pkg.time_window;
        
        pkg.time_of_interest                = [pkg.time_of_interest tmp_win];
        pkg.time_window                     = [pkg.time_window pkg.time_window];
        
        pkg.covariance_window               = [-0.4 0.6];
        ...
        pkg.cond_ix_sub                     = {'','V','N','L','R','NL','NR'};
        
        pkg.cond_ix_cue                     = {0:2,[1 2],0,1,2,0,0};
        pkg.cond_ix_dis                     = {0,0,0,0,0,0,0};
        pkg.cond_ix_tar                     = {1:4,1:4,1:4,1:4,1:4,[1 3],[2 4]};
