...
for nsuj = 1:length(suj_list)
    ...
    frequency_list                              = {'broadband' 'theta.minus1f' 'alpha.minus1f' 'beta.minus1f' 'gamma.minus1f'}; %{'theta' 'alpha' 'beta' 'gamma'};
        ...
        for ncue = 1:length(list_cue)
            ...
            for ntask = 1:length(list_task)
                for ndeco = 1:length(list_decode)
                    ...
                    %                         ext_cue '.'  ext_task '.decoding.1stgab.' ext_feat '.correct.ninjauc.mat'];
                    %                     
                    
                    fname = ['~/Dropbox/project_me/data/bil/decode/' subjectName '.1stgab.lock.' frequency_list{nfreq} ...
                        '.centered.cue.' ext_cue  '.' ext_task '.decoding.1stgab.' ext_feat '.correct.ninjauc.bsl.mat'];
            ...
            avg                         = []; clc;
            avg.label                   = {'attended' 'unattended'};
            avg.dimord                 	= 'chan_time';
            avg.time                   	= time_axis;
            avg.avg                    	= [mean(avg_data{1},1);mean(avg_data{2},1)]; clear avg_data;
            
            alldata{nsuj,nfreq,ncue}  	= avg; clear all_scores avg;
...
for nfreq= 1:size(alldata,2)
    ...
    list_name{nfreq}            = {[frequency_list{nfreq}] [' pre (b) - retro (r)']};
...
zlimit                          = [0.7 0.7 0.7 0.7 0.7];
plimit                          = 0.15;

for nfreq= 1:length(stat)
    
    if min_p(nfreq) < plimit
        ...
        for nchan = 1:length(nw_stat.label)
            ...
            if flg > 1
                ...
                subplot(nrow,ncol,i);
                h_plotSingleERFstat_selectChannel_nobox(cfg,nw_stat,squeeze(alldata(:,nfreq,:)));
                
                %                 ylabel({nw_stat.label{nchan}, ['p= ' num2str(round(min_p(nfreq),3))]});
                %                 title(list_name{nfreq});
                
                title({nw_stat.label{nchan}, ['p= ' num2str(round(min_p(nfreq),3))]});
                ylabel(list_name{nfreq});
