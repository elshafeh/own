function in_function_computeTFRMinusEvoked(data,fname_in)

avg    = ft_timelockanalysis([],data);

for n = 1:length(data.trial)
    data.trial{n} = data.trial{n}-avg.avg;
end
...
fname_out = ['../data/tfr/' fname_in '.' ext_trials '.MinusEvoked.' ext_method upper(cfg.output) '.' ext_freq '.' ext_time '.mat'];
