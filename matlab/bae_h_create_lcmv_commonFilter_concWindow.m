...
for ntrial = 1:length(data_filter.time)
    data_filter.time{ntrial} = 1:length(data_filter.time{ntrial});
end
...
for nt = 1:length(time_list)
    ...
    if time_list(nt)+time_window(nt) < 0
        tm_ext      = ['m' num2str(abs(time_list(nt)+time_window(nt))*1000)];
    else
        tm_ext      = ['p' num2str(abs(time_list(nt)+time_window(nt))*1000)];
    end
...
fname_out           = ['../data/' pkg.suj '/field/' pkg.suj '.' pkg.cond_main '.' filter_name 'ms.' pkg.lambda '.mat']; clear filter_name ;
