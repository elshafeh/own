    ...
    for ncue = 1:length(lst_cnd)
        for nmethod = 1:length(lst_mth)
            for nchan = 1:length(lst_chn)
                ...
                pow       = zeros(41,length(lst_tme));
                bsl       = zeros(41,length(lst_tme));
                ...
                for ntime = 1:length(lst_tme)
                    ...
                    lmf1                                        = find(seymour_pac.pha_freq_vec == 9);
                    lmf2                                        = find(seymour_pac.pha_freq_vec == 9);
                    act                                         = squeeze(mean(seymour_pac.mpac_norm(:,lmf1:lmf2),2));
                    ...
                    bsl                                         = squeeze(mean(seymour_pac.mpac_norm(:,lmf1:lmf2),2));
