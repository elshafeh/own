...
for ns = 1:length(list_suj)
    ...
    for ndata = 1:length(list_orig)
        ...
        for nfeat = 1:2
            ...
            fname_in                    = ['../data/coef/yc' num2str(list_suj(ns)) '.' list_orig{ndata} '.' list_feat{nfeat} '.lp30.coef.mat'];
            fprintf('\nLoading %50s\n',fname_in);
            load(fname_in);
            ...
            fname_out                   = ['../data/wt_lcmv/yc' num2str(list_suj(ns)) '.' list_orig{ndata} '.' list_feat{nfeat} '.lp30.wt_lcmv.mat'];
            fprintf('saving %50s\n',fname_out);
            save(fname_out,'data');
