...
load('J:/temp/nback/data/sens_level_auc/rt/sub43.decoding.rt.2back.dwn70.bsl.auc.mat');
template_time                           = time_axis; clear time_axis scores;

for nsuj = 1:length(suj_list)
    for nback = [0 1 2]
        
        fname                           = ['J:/temp/nback/data/sens_level_auc/rt/sub' num2str(suj_list(nsuj)) '.decoding.rt.' ... 
            num2str(nback) 'back.dwn70.bsl.auc.mat'];
        
        fprintf('loading %s\n',fname);
        load(fname);
        
        avg                             = [];
        avg.time                        = template_time;
        avg.label                       = {'auc'};
