...
for nsuj = [1:33 35:36 38:44 46:51]
    ...
    for nsess = 1:2
        ...
        chk                                                     = []; % dir(['../data/stacked/data_sess' num2str(nsess) '_s' num2str(ns) '_3stacked_excl.mat']);
        ...
        if isempty(chk)
            ...
            slct_trl                                            = find(data.trialinfo(:,4) == 0);
            cfg                                                 = [];
            cfg.trials                                          = slct_trl;
            data                                                = ft_selectdata(cfg,data);
            
            % separate condition into blocks so that stim are not 
            % confused 
            ...
            fname_out                                           = ['../data/prepro/stack_exl/sub' num2str(nsuj) '.sess' num2str(nsess) '.stk.exl.mat'];
            fprintf('Saving %s\n',fname_out);tic;
            ...
            h_erf_compute(data_repair,nsuj,nsess,'stk.exl');
            h_mtm_compute(data_repair,nsuj,nsess,1:30,'stk.exl');
