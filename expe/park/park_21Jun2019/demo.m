function [experimentResults]=rhythmInterval_speeded_demo(subjectNum,demo_blocktype)
...
% 4. Add possibility to set rhythm conidtion from the start.
...
for currentBlock = 1:length(blocks)
    ...
    for expTrial=1:currentBlockLength % loop on trials within experimental block
        ...
        switch mixedBlock(expTrial,1)
            case 1
                [resp, RT, trialTargetFrame] = trialProcRhythm(mixedBlock(expTrial,2),mixedBlock(expTrial,3),1);
            case 2
                [resp, RT, trialTargetFrame] = trialProcIntervalEmpty(mixedBlock(expTrial,2), mixedBlock(expTrial,3),1);
            case 3
                [resp, RT, trialTargetFrame] = trialProcRhythm(mixedBlock(expTrial,2),mixedBlock(expTrial,3),0);
        end
...
%%
function [resp, RT, targetIntervalFrames] = trialProcRhythm(cueISI, tarISI, trialType)
...
for frame=1:trialTargetFrame-1 %loop on frames
    ...
    keyDown=KbCheck; %check for early response
    if keyDown
...
%%
function [resp, RT, targetIntervalFrames] = trialProcIntervalEmpty(cueISI, tarISI, trialType)
...
for frame=1:trialTargetFrame-1 %loop on frames
    ...
    Screen('Flip', params.Screen.window, [], 1, 0);
