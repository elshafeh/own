...
lck_list                                = list(['1stgab.lock.theta.centered','1stgab.lock.alpha.centered','1stgab.lock.beta.centered','1stgab.lock.gamma.centered',
                                            '2ndgab.lock.theta.centered','2ndgab.lock.alpha.centered','2ndgab.lock.beta.centered','2ndgab.lock.gamma.centered'])
...
for isub in range(len(suj_list)):
    ...
    for ilock in range(len(lck_list)):    
        ...
        for ifeat in [0,1]:
            ...
            lm_time1                = np.squeeze(np.where(time_axes == -0.1))
            lm_time2                = np.squeeze(np.where(time_axes == 2))
            ...
            fname_out               = dir_data_out + suj + ext_name + '.decodinggabor.' + dcd_list[ifeat] + '.correct.timegen.mat'
