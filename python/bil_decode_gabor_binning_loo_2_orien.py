...
lck_list                                            = list(['2ndgab'])
dcd_list                                            = list(["orientation"])
...
for ilock in range(len(lck_list)): 
    ...
    for ifeat in range(len(dcd_list)):
        ...
        if dcd_list[ifeat] == "orientation":
            # orientation
            y                                       = broad_events[:,flg_gab[0]]
            y[np.where(y < 80)]                     = 0
            y[np.where(y > 80)]                     = 1
            
        elif dcd_list[ifeat] == "frequency":
            # frequeny
            y                                       = broad_events[:,flg_gab[1]]
            y[np.where(y < 0.4)]                    = 0
            y[np.where(y > 0.4)]                    = 1
        ...
        checkname                                       = dir_data_ou + suj + ext_name + '.decoding.' + dcd_list[ifeat] + '.leaveone.mat'
        
        if not os.path.exists(checkname):
            for ich in range(n_times):
                
                savename                                = dir_data_ou + suj + ext_name + '.decoding.' + dcd_list[ifeat] + '.t' + str(ich) + '.leaveone.mat'
