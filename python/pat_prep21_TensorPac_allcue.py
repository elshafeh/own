...
for s in suj_list:
    ...
    for n_method in np.array([1,2,3,4,5]): 
        for ncue in range(0,1):
            for nti in range(0,len(time1_list)):
                ...
                for xi_chan in range(0,len(chan_interest)):
                    ...
                    data_pha    = np.squeeze(low_freq_data[:,chan_interest[xi_chan],:])
                    data_pha    = data_pha[:,range(lm1,lm2)]
                    
                    data_amp    = np.squeeze(high_freq_data[:,chan_interest[xi_chan],:])
