...
for isub in range(len(suj_list)):
    ...
    for ifreq in range(len(freq_list)):
        ...
        # !! apply baseline !! #
        time_axis   = np.arange(-1,1.05,0.05)
        b1          = epochs.times[np.squeeze(np.where(np.round(time_axis,2) == np.round(-0.2,2)))]
        b2          = epochs.times[np.squeeze(np.where(np.round(time_axis,2) == np.round(0,2)))]
        epochs      = epochs.apply_baseline(baseline=(b1,b2))
