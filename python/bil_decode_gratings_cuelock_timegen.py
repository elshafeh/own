...
freq_list                                   = list(["theta.minus1f","alpha.minus1f","beta.minus1f","gamma.minus1f"])

#,"theta.minus1f","alpha.minus1f","beta.minus1f","gamma.minus1f"])
...
for isub in range(len(suj_list)):
    for ifreq in range(len(freq_list)):
        ...
        t2                                  = np.squeeze(np.where(np.round(time_axis,3) == np.round(4,3)))
        
        time_axis                           = np.squeeze(time_axis[t1:t2])
        
        ## pick correct trials
        find_correct                        = np.where(allevents[:,15] == 1)
        alldata                             = np.squeeze(alldata[find_correct,:,t1:t2])
        ...
        for ngab in [0]: 
            ...
            for ifeat in range(len(cue_list)):
                ...
                fname_out                   = fname_out + '.decoding.' + gab_list[ngab] + '.' + feat_list[ifeat] + '.correct.ninj.bsl.timegen.mat'
