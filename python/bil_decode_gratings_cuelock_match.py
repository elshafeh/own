...
freq_list                                   = list(["theta.centered","alpha.centered","beta.centered","gamma.centered"])
...
for isub in range(len(suj_list)):
    for ifreq in range(len(freq_list)):
        ...
        ext_name                                = '.1stcue.lock.' + freq_list[ifreq]
        
        dir_data_in                             = 'F:/bil/' +  'preproc/'
        ...
        list_match                              = list(["nonmatch","match"])      
        ...
        for nmatch in [0]:
            ...
            for ngab in range(len(gab_list)):
                ...
                for ifeat in range(len(cue_list)):
                    ...
                    # choose trials according to cue AND MATCHING OR NOT
                    filter_trials               = np.where((allevents[:,6]==filter_task) & (allevents[:,7]==filter_cue) & (allevents[:,5]==nmatch))
                    ...
                    fname_out                   = fname_out + '.decoding.' + gab_list[ngab] + '.' + feat_list[ifeat]
                    fname_out                   = fname_out + '.correct.' + list_match[nmatch] +'.ninjauc.mat'
